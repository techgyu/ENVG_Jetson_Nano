...
results = model("1_project_preparation/2_single_module_experiment/1_Desktop_YOLO/data/2_test_yolo11n_segmentation",
    show_boxes=False
)
...
for i, result in enumerate(results):
    if result.masks is not None and result.names is not None:
        # 원본 이미지 불러오기
        img = cv2.imread("1_project_preparation/2_single_module_experiment/1_Desktop_YOLO/data/image/bus.jpg")
        ...
        # 결과 이미지 저장
        cv2.imwrite(f"output_contour_person_{i}.jpg", img)